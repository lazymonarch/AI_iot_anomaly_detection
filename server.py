...
MODEL_PATH = "trained_smartplug_model.joblib"
SCALER_PATH = "smartplug_scaler.joblib"
...
NORMAL_DEVICE_IDS = ["Device_ESP32"]  # Add all allowed IDs here

# Prediction Function
def predict_anomaly(power_w, voltage_v, current_a):
    """Predict if incoming data is normal or a DDoS attack."""
    try:
        # Ensure the data is in the correct format for the model
        data_point = pd.DataFrame([[power_w, voltage_v, current_a]], columns=['power_w', 'voltage_v', 'current_a'])

        # Scaling the data
        scaled_data = scaler.transform(data_point)
        print(f"\n🔬 Scaled Data Sample: {scaled_data}")

        # Model Prediction
        prediction = model.predict(scaled_data)

        return "ddos_attack" if prediction[0] == 1 else "normal"
...
def is_ddos_attack(device_id, timestamp, power_w, voltage_v, current_a):
    """Check for DDoS attack using AI-based anomaly detection AND frequency."""
    anomaly_label = predict_anomaly(power_w, voltage_v, current_a)

    if anomaly_label == "ddos_attack":
        ...
        if request_count >= 2:
            ...
            if time_span > 0:
                ...
                print(f"📊 AI+Rate: {request_rate:.1f} requests/second over {time_span:.1f} seconds")
                if request_rate > 5:
                    return True
...
def is_spoofing_attack(device_id):
    if "_" in device_id and device_id not in NORMAL_DEVICE_IDS:
        return True
    return False
...
def receive_data():
    ...
    power_w = data.get("power_w", 0)  # Extract power
    voltage_v = data.get("voltage_v", 0)  # Extract voltage
    current_a = data.get("current_a", 0)  # Extract current
    ...
    if device_id in ISOLATED_DEVICES:
        if not check_device_health(device_id, power_w): # Using Power instead of the previous 'sensor_value'
            return jsonify({
                "status": "ISOLATED",
    ...
    # Using the modified "is_ddos_attack" with AI-based check + frequency check
    if is_ddos_attack(device_id, timestamp, power_w, voltage_v, current_a):
    ...
    if is_replay_attack(device_id, power_w): # Using Power
    ...
    if power_w > THRESHOLD_SENSOR: # Using Power
        print(f"⚠️ Malicious data detected from {device_id} ({client_ip}) (Value: {power_w})")
        block_ip(client_ip)
